...
import { useAddDailyPnL } from "@/hooks/usePnLData";
...
export const AddPnLDialog = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await addPnL.mutateAsync({
        trade_date: format(date, "yyyy-MM-dd"),
        pnl: pnlValue,
      });
      toast.success("P&L entry added successfully");
      setOpen(false);
      setPnl("");
      setDate(new Date());
    } catch (error: any) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[400px] bg-card border-border">
        <DialogHeader>
          <DialogTitle className="text-foreground">Add Daily P&L Entry</DialogTitle>
          <DialogDescription className="text-muted-foreground">
            Enter your profit or loss for a trading day.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-6 mt-4">
          ...
          <Button
            ...
            disabled={addPnL.isPending}
          >
            {addPnL.isPending ? "Adding..." : "Add Entry"}
          </Button>
        </form>
      </DialogContent>
    </Dialog>
